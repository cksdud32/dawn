  ...
  if (!public_ && !isOwn) {
    return (
      <div className="min-h-screen bg-[#faf8f4] dark:bg-[#07070e] flex flex-col items-center justify-center gap-4 text-center px-6">
        ...
        <p className="text-stone-500 dark:text-white/50">아직 공개되지 않은 글입니다</p>
        <Link href="/" className="text-sm text-stone-400 dark:text-white/30 hover:text-stone-600 dark:hover:text-white/50 underline underline-offset-4">
          돌아가기
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#faf8f4] dark:bg-[#07070e] text-stone-800 dark:text-white">
      <div className="max-w-xl mx-auto px-4 py-10">
        <Link href="/" className="text-sm text-stone-400 dark:text-white/30 hover:text-stone-600 dark:hover:text-white/50 mb-8 block">
          ← 돌아가기
        </Link>
        ...
        <article className="space-y-4 mb-12">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-stone-300 dark:text-white/25 ml-auto">{timeStr}</span>
          </div>

          <p className="text-stone-700 dark:text-white/80 leading-relaxed text-[16px] font-light whitespace-pre-wrap">
            {post.content}
          </p>

          {post.musicTitle && (
            <div className="flex items-center gap-2 text-sm text-stone-400 dark:text-white/35 border-t border-stone-200 dark:border-white/8 pt-3">
              <span>♪</span>
              <span>{post.musicTitle}{post.musicArtist ? ` — ${post.musicArtist}` : ""}</span>
            </div>
          )}

          <p className="text-xs text-stone-300 dark:text-white/20">{post.dawnDate} 새벽의 기록</p>
        </article>
